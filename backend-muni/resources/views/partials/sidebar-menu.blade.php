...
<!-- Mesa de Partes -->
<div x-data="{ open: {{ str_contains($currentRoute, 'mesa-partes') ? 'true' : 'false' }} }">
    <button @click="open = !open" 
            class="@if(str_contains($currentRoute, 'mesa-partes')) text-municipal-700 @else text-gray-700 hover:text-municipal-700 @endif group flex items-center w-full {{ $mobilePadding }} {{ $mobileClass }} rounded-md transition duration-200">
        <svg class="@if(str_contains($currentRoute, 'mesa-partes')) text-municipal-500 @else text-gray-400 group-hover:text-municipal-500 @endif mr-3 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div x-show="open" x-transition class="ml-6 mt-1 space-y-1">
        <a href="{{ route('mesa-partes.index') }}" 
           class="@if($currentRoute == 'mesa-partes.index') text-municipal-600 bg-municipal-50 @else text-gray-600 hover:text-municipal-600 @endif group flex items-center py-2 px-2 text-sm rounded-md">
            Dashboard Mesa de Partes
        </a>
        <a href="{{ route('mesa-partes.registro') }}" 
           class="@if($currentRoute == 'mesa-partes.registro') text-municipal-600 bg-municipal-50 @else text-gray-600 hover:text-municipal-600 @endif group flex items-center py-2 px-2 text-sm rounded-md">
            Registrar Documento
        </a>
        <a href="{{ route('mesa-partes.derivacion') }}" 
           class="@if($currentRoute == 'mesa-partes.derivacion') text-municipal-600 bg-municipal-50 @else text-gray-600 hover:text-municipal-600 @endif group flex items-center py-2 px-2 text-sm rounded-md">
            Derivación
        </a>
    </div>
</div>
...
<a href="{{ route('profile') }}" 
   class="@if($currentRoute == 'profile') bg-municipal-100 text-municipal-700 border-r-4 border-municipal-600 @else text-gray-700 hover:bg-municipal-50 hover:text-municipal-700 @endif group flex items-center {{ $mobilePadding }} {{ $mobileClass }} rounded-md transition duration-200">
    <svg class="@if($currentRoute == 'profile') text-municipal-500 @else text-gray-400 group-hover:text-municipal-500 @endif mr-3 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
